...
def main():
    ...
    if(len(sys.argv)==2):
        if(sys.argv[1]=="--h" or sys.argv[1]=="--H" ):
            print("This script is used to perform to copy files from one Directory to another")
            exit()

        if(sys.argv[1]=="--u" or sys.argv[1]=="--U"):
            print("Use of script :")
            print("Name_of_file Src_Directory Des_Directory")
            exit()
    if(len(sys.argv)==3):
        try:
            starttime=time.time()
            DirectoryAutomation.CopyFilesToDir(fobj,sys.argv[1],sys.argv[2])
            endtime=time.time()
