...
def main():
    ...
    if(len(sys.argv)==2):
        if(sys.argv[1]=="--h" or sys.argv[1]=="--H" ):
            print("This script is used to perform to change extention")
            exit()

        if(sys.argv[1]=="--u" or sys.argv[1]=="--U"):
            print("Use of script :")
            print("Name_of_file Name_of_Directory Src_File_Extention Des_File_Extention")
            exit()
    if(len(sys.argv)==4):
        try:
            starttime=time.time()
            DirectoryAutomation.ChangeFileExtention(fobj,sys.argv[1],sys.argv[2],sys.argv[3])
            endtime=time.time()
