...
def main():
    ...
    if(len(sys.argv)==2):
        if(sys.argv[1]=="--h" or sys.argv[1]=="--H" ):
            print("This script is used to perform to copy givem extention files from one Directory to another")
            exit()

        if(sys.argv[1]=="--u" or sys.argv[1]=="--U"):
            print("Use of script :")
            print("Name_of_file Src_Directory Des_Directory File_Extention")
            exit()
    if(len(sys.argv)==4):
        try:
            starttime=time.time()
            DirectoryAutomation.CopyFilesToDirByExtention(fobj,sys.argv[1],sys.argv[2],sys.argv[3])
            endtime=time.time()
